"""GCN training variant: precomputed tensors + negative subsampling.

Like ``train_gnn.py`` but loads pre-saved PyTorch graph tensors from
``./saved_tensors/`` (instead of rebuilding from pickles) and subsamples a
single negative graph per file to balance against the positives.

The methodology fixes from ``train_gnn.py`` apply here too: a structure-level
(PDB-grouped) split, a held-out structure kept out of the corpus, imbalance-
aware metrics, and a fixed seed. See that file for details.
"""
import copy
import glob
import os
...
def random_sample_one(graphs):
    """Subsample a single negative graph per file to balance the dataset."""
    return random.sample(graphs, 1) if graphs else []
...
pos_list, neg_list = [], []
for line in lines:
    pos_list.extend(glob.glob(TENSOR_DIR + line + "pos_graph.pt"))
    neg_list.extend(glob.glob(TENSOR_DIR + line + "neg_graph.pt"))

data_list = []
for path in neg_list:
    data_list.extend(tag(random_sample_one(torch.load(path)), pdb_from_tensor_path(path)))
