...
pickle files (one NetworkX adjacency/feature dict per binding-site candidate),
converts them to PyTorch Geometric graphs, trains a GCN graph classifier, and
saves the best-performing weights to ``best_model.pth``. It also evaluates the
trained model on the held-out 4RUM structure and writes ROC predictions.
"""
...
# Single held-out structure used for evaluation.
PDB_4RUM_PICKLE = "./4RUM.pkl"

# If you use GPU, the device should be cuda
print('Device: {}'.format(device))
pos_pickles=(glob.glob(os.path.join(PICKLE_DIR, "*_pos.pkl")))
neg_pickles=(glob.glob(os.path.join(PICKLE_DIR, "*_neg.pkl")))
...
for i in neg_pickles:
	...
	for coord,graph in (newdict2.items()):
		...
		pyg_graph.pdb= i[-12:-8]
		data_list.append(pyg_graph)
with open(PDB_4RUM_PICKLE, 'rb') as f:
	    newdict2 = pickle.load(f)
for coord,graph in (newdict2.items()):
		G=nx.from_numpy_array(np.array(graph['A1']))
		for node in G.nodes:
