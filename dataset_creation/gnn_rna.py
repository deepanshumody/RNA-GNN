# -*- coding: utf-8 -*-
"""Convert RNA PDB structures into candidate-site subgraph pickles.

For each RNA structure in ``RNA-only-PDB``, a 3 Angstrom grid is laid over
the molecule's bounding box. Each grid point is treated as a candidate ion
binding site: the ideal magnesium ion model (``MG_ideal.sdf``) is placed at
the grid point and a receptor-ligand subgraph is built from receptor atoms
within range. A grid point is labelled positive (``C == 1``) when it is the
nearest grid point to an actual ion observed in the structure, and negative
otherwise. The resulting subgraph samples (atom features, adjacency matrices,
validity mask, and label) are pickled per structure into ``RNA-graph-pickles``.
"""
...
for pdbfile in pdbfiles:
    ...
    newlist=[]
    for i in range(int(round_to_3(min(d1[:,0])))-6,int(max(d1[:,0]))+7,3):
      for j in range(int(round_to_3(min(d1[:,1])))-6,int(max(d1[:,1]))+7,3):
        for k in range(int(round_to_3(min(d1[:,2])))-6,int(max(d1[:,2]))+7,3):
    ...
    with open('RNA-graph-pickles/'+pdbname+'.pkl', 'wb') as file:	  

      pickle.dump(pdbdict, file)
    with open('RNA-graph-pickles/'+pdbname+'_pos.pkl', 'wb') as file:	  

      pickle.dump(pdbdictpos, file)
    with open('RNA-graph-pickles/'+pdbname+'_neg.pkl', 'wb') as file:	  
